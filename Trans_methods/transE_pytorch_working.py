...
class TransEModel(nn.Module):
	def __init__(self, model_options):
		...
		self.learning_rate = model_options.learning_rate
		self.early_stopping_round = model_options.early_stopping_round
		self.L1_flag = model_options.L1_flag
		self.filter = model_options.filter
		self.embedding_size = model_options.embedding_size
		self.entity_total = model_options.entity_total
		self.relation_total = model_options.relation_total
		self.batch_size = model_options.batch_size
...
class Model_config(object):
	...
	def evaluation_transE_helper(testList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=0):
...
if __name__ == "__main__":
	...
	model_options.loss_function = marginLoss()
	...
	model = TransEModel(model_options)
	...
	filename = '_'.join(
		['l', str(model_options.learning_rate),
		 ...
		 'lo', str(model_options.loss_type),]) + '_TransE.ckpt'
	...
	for epoch in range(model_options.train_times):
		...
		if model_options.early_stopping_round > 0:
			if epoch == 0:
				...
				filter = model.filter	
				hit10, best_meanrank = model_options.evaluation_transE(validList, tripleDict, ent_embeddings, rel_embeddings, 
					L1_flag, filter, model_options.batch_size)
			...
			# Evaluate on validation set for every 5 epochs
				...
				L1_flag = model.L1_flag
				filter = model.filter
				hit10, now_meanrank = model_options.evaluation_transE(validList, tripleDict, ent_embeddings, rel_embeddings, 
					L1_flag, filter, model_options.batch_size)
	...
	hit10Test, meanrankTest = model_options.evaluation_transE(testList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=0)

	hit10OneToOneHead, meanrankOneToOneHead = model_options.evaluation_transE(oneToOneList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=1)
	hit10OneToManyHead, meanrankOneToManyHead = model_options.evaluation_transE(oneToManyList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=1)
	hit10ManyToOneHead, meanrankManyToOneHead = model_options.evaluation_transE(manyToOneList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=1)
	hit10ManyToManyHead, meanrankManyToManyHead = model_options.evaluation_transE(manyToManyList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=1)

	hit10OneToOneTail, meanrankOneToOneTail = model_options.evaluation_transE(oneToOneList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=2)
	hit10OneToManyTail, meanrankOneToManyTail = model_options.evaluation_transE(oneToManyList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=2)
	hit10ManyToOneTail, meanrankManyToOneTail = model_options.evaluation_transE(manyToOneList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=2)
	hit10ManyToManyTail, meanrankManyToManyTail = model_options.evaluation_transE(manyToManyList, tripleDict, ent_embeddings, rel_embeddings, L1_flag, filter, head=2)	
	...
	with open(os.path.join('./result/', 'FB15k.txt'), 'a') as fw:
