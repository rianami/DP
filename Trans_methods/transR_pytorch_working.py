...
class Model_config(object):
	...
	def evaluation_transR_helper(testList, tripleDict, ent_embeddings,rel_embeddings, proj_embeddings, L1_flag, filter, head):
	...
	def evaluation_transR(testList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, k=0, num_processes=multiprocessing.cpu_count(), head=0):
		# embeddings are torch tensor like (No Variable!)
...
if __name__ == "__main__":
	...
	trainTotal, trainList, trainDict = model_options.loadTriple('./data/FB15k', 'train2id.txt')
	validTotal, validList, validDict = model_options.loadTriple('./data/FB15k', 'valid2id.txt')
	tripleTotal, tripleList, tripleDict = model_options.loadTriple('./data/FB15k', 'triple2id.txt')
	...
	model = model.TransRModel(model_options)
	...
	filename = '_'.join(
		['l1', str(model_options.learning_rate1),
		 'l2', str(model_options.learning_rate2),
		 'es', str(model_options.early_stopping_round),
		 'L', str(model_options.L1_flag),
		 'eem', str(model_options.embedding_size),
		 'rem', str(model_options.embedding_size),
		 ...
		 'lo', str(model_options.loss_type),]) + '_TransR.ckpt'
	...
	for epoch in range(model_options.train_times):
		...
		for batchList in trainBatchList:
			...
			losses = losses + model_options.normLoss(ent_embeddings) + model_options.normLoss(rel_embeddings) + model_options.normLoss(pos_h_e) + model_options.normLoss(pos_t_e) + model_options.normLoss(neg_h_e) + model_options.normLoss(neg_t_e)
		...
		if model_options.early_stopping_round > 0:
			if epoch == 0:
				...
				filter = model.filter	
				hit10, meanrank = model_options.evaluation_transR(validList, tripleDict, ent_embeddings, rel_embeddings, 
					proj_embeddings, L1_flag, filter, k=model_options.batch_size)
				worst_hit10 = hit10
			...
			# Evaluate on validation set for every 5 epochs
				...
				if phase == 0:
					...
					# and the second phase begins.
						...
						phase += 1
						optimizer.param_groups[0]['lr'] = model_options.learning_rate2
						best_epoch = epoch
	...
	testTotal, testList, testDict = model_options.loadTriple('./data/FB15k', 'test2id.txt')
	oneToOneTotal, oneToOneList, oneToOneDict = model_options.loadTriple('./data/FB15k', 'one_to_one.txt')
	oneToManyTotal, oneToManyList, oneToManyDict = model_options.loadTriple('./data/FB15k', 'one_to_many.txt')
	manyToOneTotal, manyToOneList, manyToOneDict = model_options.loadTriple('./data/FB15k', 'many_to_one.txt')
	manyToManyTotal, manyToManyList, manyToManyDict = model_options.loadTriple('./data/FB15k', 'many_to_many.txt')
	...
	hit10Test, meanrankTest = model_options.evaluation_transR(testList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=0)

	hit10OneToOneHead, meanrankOneToOneHead = model_options.evaluation_transR(oneToOneList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=1)
	hit10OneToManyHead, meanrankOneToManyHead = model_options.evaluation_transR(oneToManyList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=1)
	hit10ManyToOneHead, meanrankManyToOneHead = model_options.evaluation_transR(manyToOneList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=1)
	hit10ManyToManyHead, meanrankManyToManyHead = model_options.evaluation_transR(manyToManyList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=1)

	hit10OneToOneTail, meanrankOneToOneTail = model_options.evaluation_transR(oneToOneList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=2)
	hit10OneToManyTail, meanrankOneToManyTail = model_options.evaluation_transR(oneToManyList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=2)
	hit10ManyToOneTail, meanrankManyToOneTail = model_options.evaluation_transR(manyToOneList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=2)
	hit10ManyToManyTail, meanrankManyToManyTail = model_options.evaluation_transR(manyToManyList, tripleDict, ent_embeddings, rel_embeddings, proj_embeddings, L1_flag, filter, head=2)	
	...
	with open(os.path.join('./result/','FB15KTRANSR.txt'), 'a') as fw:
